...
Robot::Robot()
{
	this->initWithFile(PATH_TO_RESOURCES + "/MenuItems/Robot.png");
...
void Robot::LoadNameSprites()
{
	...
	{
		m_vecDead.push_back(PATH_TO_RESOURCES	+ "/Heroes/Robot/Dead/Dead_"	+ std::to_string(i) + ".png");
		m_vecIdle.push_back(PATH_TO_RESOURCES	+ "/Heroes/Robot/Idle/Idle_"	+ std::to_string(i) + ".png");
		m_vecJump.push_back(PATH_TO_RESOURCES	+ "/Heroes/Robot/Jump/Jump"		+ std::to_string(i) + ".png");
		m_vecSlide.push_back(PATH_TO_RESOURCES	+ "/Heroes/Robot/Slide/Slide_"	+ std::to_string(i) + ".png");
	}
	for (int i = 1; i <= SPRITES_NINE; i++)
	{
		m_vecShootRun.push_back(PATH_TO_RESOURCES + "/Heroes/Robot/RunShoot/RunShoot_" + std::to_string(i) + ".png");
	}
	for (int i = 1; i <= SPRITES_EIGHT; i++)
	{
		m_vecMeleeJump.push_back(PATH_TO_RESOURCES	+ "/Heroes/Robot/JumpMelee/JumpMelee_"	+ std::to_string(i) + ".png");
		m_vecMelee.push_back(PATH_TO_RESOURCES		+ "/Heroes/Robot/Melee/Melee_"			+ std::to_string(i) + ".png");
		m_vecRun.push_back(PATH_TO_RESOURCES		+ "/Heroes/Robot/Run/Run_"				+ std::to_string(i) + ".png");
	}
	for (int i = 1; i <= SPRITES_FIVE; i++)
	{
		m_vecShootJump.push_back(PATH_TO_RESOURCES + "/Heroes/Robot/JumpShoot/JumpShoot_" + std::to_string(i) + ".png");
	}
	for (int i = 1; i <= SPRITES_FOUR; i++)
	{
		m_vecShoot.push_back(PATH_TO_RESOURCES + "/Heroes/Robot/Shoot/Shoot_" + std::to_string(i) + ".png");
	}
}
...
{
	this->setTexture(CCTextureCache::sharedTextureCache()->addImage(m_vecMeleeJump[m_indexMeleeJump]));
...
{
	this->setTexture(CCTextureCache::sharedTextureCache()->addImage(m_vecShootJump[m_indexShootJump]));
...
{
	this->setTexture(CCTextureCache::sharedTextureCache()->addImage(m_vecShootRun[m_indexShootRun]));
